...
async function createPostIt(userId, categoryId, title, description, pinned, weekday) {
    ...
    const selectTitleQuery = `SELECT * FROM post_it WHERE title='${title}' AND user_id='${userId}';`;
    const selectAllPostItDay = `SELECT * FROM post_it WHERE user_id='${userId}' AND day_of_week = '${weekday}';`;
    ...
    try {
        ...
        const countOfPostItForDay = await connection.execute(selectAllPostItDay);

        if(countOfPostItForDay[0].length >= MAX_POST_IT)
        {
            logger.info(`Already 20 post-it for today. Cannot create more.`);
            exception = new postItException(`Already 20 post-it for today. Cannot create more.`);
            return exception;
        }
 ...
 * @throws User.databaseException, postItException
 */
 async function findByWeekdayAndUserId(weekday, userId, pinned) {
    ...
    const selectQuery = `SELECT * FROM post_it WHERE day_of_week='${weekday}' AND user_id='${userId}' AND completed = 'F' AND pinned = '${pinned}';`;
    try {
...
/**
 * Validates and updates the specified post-it as completed.
 * @param {*} postItId Post-It id for the post that needs to be marked as completed.
 * @returns 
 */
